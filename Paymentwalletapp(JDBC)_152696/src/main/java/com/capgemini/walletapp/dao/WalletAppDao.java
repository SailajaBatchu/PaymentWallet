...
public class WalletAppDao implements IWalletAppDAO {
	...
	public boolean createAccount(Customer customer) {
		...
		try {
			...
			pstmt2.setLong(3, customer.getAdharNumber());
	...
	public void printTrans() {
		...
		try {
			if (rs2.first()) {
				String select="select * from transaction where adharNum="+rs2.getDouble(4);
				java.sql.PreparedStatement stm = con.prepareStatement(select);
